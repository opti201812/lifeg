...
import { RADAR_DATA_EXPIRE_TIME } from "../utils/constants";
...
export const processPersonnelDeviceData = (
   ...
   // 🔥 手环数据
   const braceletHeartRate = braceletData?.heartRate || "-";
   const systolicPressure = braceletData?.systolicPressure || "-";
   const diastolicPressure = braceletData?.diastolicPressure || "-";
   const spo2 = braceletData?.spo2 || braceletData?.bloodOxygen || "-";
   const bodyTemperature = braceletData?.bodyTemperature || "-";
